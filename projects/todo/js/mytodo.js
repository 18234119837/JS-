...
$('#subimt').click(function () {
    ...
    data.push({con: text,
        time, isStart: 0, isDone: 0, color,kuaii:0})
    saveData(data);
    reWrite();
    $('.editarea')
        .removeClass('show')
    ...
    $.each(data, function (index, val) {
        if (state == 'wait') {//如果是未完成的
            if (val.isDone == 0) {
                ...
                str += "<li id='" + index + "'  style='background:" + val.color + "'><div class='kuai  "+addclassname+"'></div><time>" + getYear(val.time) + "<br><span>" + gettime(val.time) + "</span></time> <p>" + val.con + "</p> <span   class='iconfont "+className+" '>&#xe662;</span><div class='right'>完成</div></li>";
            }
        }
            ...
            if (val.isDone == 1) {
                ...
                str += "<li id='" + index + "'   style='background:#CCCCCC'><div class='kuai"+addclassname+"'></div><time>" + getYear(val.time) + "<br><span>" + gettime(val.time) + "</span></time> <p>" + val.con + "</p><span class='iconfont "+className+" '>&#xe662;</span><div class='del'>删除除</div></li>";
            }
        }
    })
    $('.content ul').html(str);
